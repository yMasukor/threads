...
//the own renderer
	...
	PhaserRender.prototype.onParticleCreated = function ( particle ) {
		...
		if ( particle.target ) {
			...
			particle.target = this.element.game.make.sprite(0,0, 'target' );
			...
			// if(particle.imageTarget){
			// 	particle.target = this.element.game.make.sprite(0,0, bmd )
			// }else{
			...
			var bmd = this.element.game.make.bitmapData(particle.radius*2, particle.radius*2);
				bmd.circle(particle.radius, particle.radius, particle.radius, particle.color);
