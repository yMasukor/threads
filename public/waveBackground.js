...
var waveBackground = {
	...
	create:function(){
		...
		for(var i=0; i<Math.floor(bufferLength*0.33); i=i+5){
		...
		j = 0
		for(var i=Math.floor(bufferLength*0.33); i<Math.floor(bufferLength*0.66); i=i+5){
		...
		j = 0
		for(var i=Math.floor(bufferLength*0.66); i<bufferLength; i=i+5){
	...
	update:function(){
		...
		for(var i=0; i<Math.floor(bufferLength*0.33); i=i+10){
			...
			var y = (view.bounds.height - ((v-128)*(1*complexity.level+1))) - (view.bounds.height*0.5) ;

			// console.log(j, this.bassPath.segments.length)

			var point = this.bassPath.segments[j+2].point;
		 	point.y -= ((point.y-y)/30);
		...
		for(var i=Math.floor(bufferLength*0.33); i<Math.floor(bufferLength*0.66); i=i+10){
			...
			var y = (view.bounds.height - ((v-128)*(2*complexity.level+1))) - (view.bounds.height*(0.5-(0.05*complexity.level))) ;
